...
//  HighRollers
//
//  Created by Austin Trumbly on 5/22/24.
//
...
struct ContentView: View {
    ...
    @State private var currentResults = DiceResult(type: 0, number: 0)
    ...
    var body: some View {
        NavigationStack {
            Form {
                Section {
                    ...
                    LazyVGrid(columns: columns) {
                        ForEach(0..<currentResults.rolls.count, id: \.self) { rollNumber in
                            Text(String(currentResults.rolls[rollNumber]))
                    ...
                    .accessibilityLabel("Latest roll: \(currentResults.description)")
                }
                .disabled(stoppedDice < currentResults.rolls.count)
            ...
            .sensoryFeedback(.impact, trigger: currentResults.rolls)
        }
    }
    
    func rollDice() {
        currentResults = DiceResult(type: selectedDiceType, number: numberToRoll)
        if accessibilityVoiceOverEnabled {
            stoppedDice = numberToRoll
            savedResults.insert(currentResults, at: 0)
            save()
        } else {
            stoppedDice = -15
        }
    ...
    func updateDice() {
        guard stoppedDice < currentResults.rolls.count else { return }
        
        for i in stoppedDice..<numberToRoll {
            if i < 0 { continue }
            currentResults.rolls[i] = Int.random(in: 1...selectedDiceType)
        }
